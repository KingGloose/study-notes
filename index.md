...
- **MySQL**:数据库/表操作、增删改查、备份恢复、事务与隔离级别、约束、多表查询/子查询
- ⭐**MySQL 索引为什么快**(`wiki/数据库/`):**成本单位是页(16KB)不是行**、500MB≈32000页、
  为什么不能用二叉树(23层+16KB只换1bit)、B+树~1170叉、三层装2000万行、
  聚簇索引(叶子页就是数据页,"找到key"="找到行")、叶子双向链表与范围查询、
  Buffer Pool 常驻非叶子层 → 实际1次IO;**纠正旧笔记「索引=二叉树」的错误**
- **MySQL 待学**:二级索引与回表、覆盖索引、最左前缀、索引失效场景、explain 执行计划
