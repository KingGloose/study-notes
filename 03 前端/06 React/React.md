...
5、我们使用 `setState` 是异步的，而且还做了数据比对，在点击之后并不是加3次，而是只执行了一次，我们如何去避免这个问题呢？

![[00 assets/7e7027588d8f9e41432af2869da4ce63_MD5.jpeg]]

`setCount`函数内部是可以传输函数的，函数参数是上一步的值，这样就可以实现数据的连续处理了

![[00 assets/8d9b294ef4912947d538ec0abea3c87c_MD5.jpeg]]
...
![[00 assets/49e379cb73677057edc5bcd443131456_MD5.jpeg]]
...
这个时候就存在一个疑问就是，以前写的函数为啥就不存在闭包陷阱呢？这个就是函数直接调用和`回调函数`的区别了，外面创建`useCallback`中回调的函数，这里面直接获取函数外部的值就是闭包处理，所以就会存在闭包陷阱的问题
...
但是我们折腾这么长时间来解决这个问题，最后还是需要重新创建回调函数的形式获取值，这还不如直接使用`const increment = () => setCount(count + 1)`来处理，其实这个优化的就是将这个函数传递给子组件的时候使用的
...
[useLayoutEffect | react docs](https://message163.github.io/react-docs/hooks/useLayoutEffect.html)
...
3、而且 `useLayoutEffect`是同步执行，并阻塞DOM的加载
![[00 assets/0a26864efaac52e357aa61158b828e0c_MD5.jpeg]]
